...
/// [RouteAware<ModalRoute<dynamic>>] and subscribing it to [FirebaseAnalyticsGoRouterObserver]. See the
/// [RouteObserver<ModalRoute<dynamic>>] docs for an example.
class FirebaseAnalyticsGoRouterObserver
  ...
  /// `debugPrint()`.
  FirebaseAnalyticsGoRouterObserver({
    ...
    if (screenName != null) {
      analytics.logScreenView(screenName: screenName).catchError(
        (Object error) {
          ...
          if (_onError == null) {
            debugPrint('$FirebaseAnalyticsGoRouterObserver: $error');
          } else {
